...
import TargetInference.utils as utils
...
def target_features(targets_sim_matrix, target_idx, target, claim):
    ...
    return [f_sim, f_spos, f_epos, f_num_words, f_tag_conf, f_clen, num_of_positive_words, num_of_negative_words,
...
def build_instance(claims, conclusion_target_txt=None):
    ...
    targets_vectors = [utils.embed_sentence(target[0]['text'], normalize=False) for target in targets]
    ...
    for idx, item in enumerate(targets):
        ...
        if conclusion_target_txt is not None:
...
def train_lambdamart(data_path, output_path):
    ...
    train_x, test_x, train_y, test_y, train_qidx, test_qidx = train_test_split(features, labels, qIdxs, test_size=0.2,
                                                                               shuffle=False)
...
def generate_training_file(path, output_path):
    ...
    with open(output_path, 'w') as f:
        for sample in data:
            ...
            conclusion_target = sample['conclusion']['targets'][0]['text'].lower() if len(
                sample['conclusion']['targets']) > 0 else sample['conclusion']['text'].lower()
...
# Simple scoring: python ranking_targets.py simple_scoring ./ ../data/idebate/test_scored.json
                       ../data/idebate/test_scored.json
"""
