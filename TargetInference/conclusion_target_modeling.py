...
from TargetInference.utils import *
from TargetInference.networks import get_triplet_model
from TargetInference import ranking_targets
import pickle


neu_templates = ['In this essays, we discussed <target>', 'I would say <target> should be discussed']
pos_templates = ['<target> is a good idea', '<target> should be recommended']
neg_templates = ['<target> is a bad idea', '<target> would lead to bad consequences']
...
class RankedTargetModel(ClaimTargetModeling):
    ...
    @staticmethod
    def get_random_target(premises):
    ...
    def get_sentiment(self, premises):
        if type(premises) == list:
            ...
            return premise_sentiment(premises)  # prev 'premises' were 'x'
...
class TargetSpaceModeling(ClaimTargetModeling):
    ...
    def __init__(self, siamese_model_path=None, load_concepts=True, embedding_method='fasttext',
                 src_target_space_path=None):
        ...
        if src_target_space_path is not None:
            print('Loading src target space from:', src_target_space_path)
            self.all_targets = json.load(open(src_target_space_path + '.json', encoding="utf-8"))
        ...
        if siamese_model_path is not None:
            ...
            self.model = get_triplet_model(siamese_model_path)
    ...
    def _accepted_target(self, target):
        return (target is not None) and (target not in self.stopwords) and (len(target.split(' ')) < 5)
    ...
    def fit_on_data(self, train_data, normalize=True):
        ...
        train_targets = set(
            [(target, s['text']) for s in (premises + conclusions) for target in self.get_premise_targets(s)])
        train_targets = set([target for target in train_targets if self._accepted_target(target[0])])

        self.builtin_concepts = set(
            [(x, x) for x in self.builtin_concepts])  # To match with the structure of claims DS...

        self.all_targets = train_targets | self.builtin_concepts

        print('Number of targets:', len(self.all_targets))
        self.src_target_space = {target[0]: embed_sentence(target[0], target[1], normalize=normalize,
                                                           embedding_method_name=self.embedding_method) for target in
                                 self.all_targets if target is not None}
        # filter none targets
        self.src_target_space = dict(
            [(key, value) for key, value in self.src_target_space.items() if value is not None])
        self.all_targets = [target for target in self.all_targets if target[0] in self.src_target_space]

        if self.model is not None:
            self.learned_target_space = {
                target[0]: self.model.get_target_embedding(
                    torch.from_numpy(self.src_target_space[target[0]]).unsqueeze(1).unsqueeze(
                        0)).detach().numpy().reshape(-1)
                for target in self.all_targets if target is not None}
        else:
            self.learned_target_space = {}
    ...
    @staticmethod
    def get_premise_targets(premise):
        return [target['text'] for target in premise['targets']]

    @staticmethod
    def get_premise_target(premise):
    ...
    @staticmethod
    def score_target(target_text, target_vec, premises, a=0.5, b=0.5):
        """
        avg_sim_to_premises = np.mean(
            [(1 - distance.cosine(target_vec, premise[1])) for premise in premises if premise[0] != target_text])
        ...
        return score
        """
        raise NotImplementedError("This method had been cut due to no usage by Torben.")

    def get_target(self, premises, simple_avg=False):
        targets = [(target['text'], p['text']) for p in premises for target in p['targets']]
        targets = list(
            map(lambda x: (x[0], embed_sentence(x[0], x[1], embedding_method_name=self.embedding_method)), targets))
        ...
        if simple_avg:
            ...
            # Normalizing..
            # mapped_conc_tensor = mapped_conc_tensor/np.linalg.norm(mapped_conc_tensor)
            # if np.linalg.norm(mapped_conc_tensor) != 0 else mapped_conc_tensor
